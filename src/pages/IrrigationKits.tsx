...
const IrrigationKits = () => {
  ...
  const { data: kits = [], isLoading, error, refetch } = useQuery({
  ...
  const processedKits = React.useMemo(() => {
    let filtered = kits.filter(kit => {
      ...
      const matchesSearch = 
        ...
        (kit.description && kit.description.toLowerCase().includes(searchText)) ||
        (kit.kit_type && kit.kit_type.toLowerCase().includes(searchText)) ||
        (kit.recommended_crops && kit.recommended_crops.some(crop => crop.toLowerCase().includes(searchText)));
      
      const matchesKitType = selectedKitType === 'all' || kit.kit_type === selectedKitType;
      const matchesComplexity = selectedComplexity === 'all' || kit.installation_complexity === selectedComplexity;
      
      const price = kit.price || 0;
      const matchesPrice = (!priceRange.min || price >= parseFloat(priceRange.min)) &&
                          (!priceRange.max || price <= parseFloat(priceRange.max));
      
      return matchesSearch && matchesKitType && matchesComplexity && matchesPrice;
    });
    ...
    switch (sortBy) {
      ...
      case 'price_high':
        ...
        break;
      case 'coverage_area':
        filtered.sort((a, b) => (b.coverage_area || 0) - (a.coverage_area || 0));
        break;
      case 'efficiency':
        filtered.sort((a, b) => (b.water_efficiency_percentage || 0) - (a.water_efficiency_percentage || 0));
        break;
      case 'newest':
    ...
    return filtered;
  }, [kits, searchTerm, selectedKitType, selectedComplexity, priceRange, sortBy]);
      ...
      <div className="min-h-screen bg-background border-t border-border">
        ...
        <main className="py-12 sm:py-16 lg:py-20">
          <div className="container mx-auto px-4 text-center">
            ...
            <h2 className="text-2xl font-bold mb-4 text-foreground">Unable to Load Kits</h2>
            <p className="text-muted-foreground mb-6">
              We're having trouble loading the irrigation kits. Please try again.
            </p>
            <Button 
              onClick={() => refetch()} 
        ...
        <div className="container mx-auto px-4">
          ...
          {isLoading && (
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-foreground">Loading irrigation kits...</h3>
            </div>
          )}
          ...
          {!isLoading && processedKits.length === 0 && (
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold mb-2 text-foreground">No kits found</h3>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-border bg-muted/30">
                        ...
                        <th className="text-center p-4 font-semibold text-foreground">Coverage</th>
                        <th className="text-center p-4 font-semibold text-foreground">Efficiency</th>
                          ...
                          <tr key={kit.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                            <td className="p-4">
                              <div className="flex items-center gap-3">
                                <div>
                                  <div className="font-semibold text-foreground">{kit.name}</div>
                            ...
                            <td className="p-4 text-right">
                              {hasPricingTiers ? (
                                <div className="text-right">
                                  <div className="font-bold text-lg text-primary">Multiple Options</div>
                                  <div className="text-sm text-muted-foreground">
                                    {formatCurrency(Math.min(...components.pricing_tiers!.map(t => t.price)))} - {formatCurrency(Math.max(...components.pricing_tiers!.map(t => t.price)))}
                                  </div>
                                </div>
                              ) : (
                                <div className="font-bold text-lg text-primary">{formatCurrency(kit.price)}</div>
                              )}
                            </td>
                      ...
                      <div className="bg-muted/30 rounded-lg p-4 border border-border">
                        {hasPricingTiers ? (
                            ...
                            <div className="space-y-2">
                              ...
                              {components.pricing_tiers!.length > 3 && (
                                <div className="text-center text-sm text-muted-foreground py-2">
                                  +{components.pricing_tiers!.length - 3} more options
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="text-center">
                            ...
                            <div className="text-sm text-muted-foreground">Complete kit price</div>
                          </div>
                        )}
                      </div>
              ...
              {selectedKit && (
                <>
                  <DialogHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        <DialogTitle className="text-3xl font-bold text-foreground">{selectedKit.name}</DialogTitle>
                        <DialogDescription className="text-lg mt-2 text-muted-foreground">
                          {selectedKit.description || 'Professional irrigation solution'}
                        </DialogDescription>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="capitalize border-border text-muted-foreground">
                          {selectedKit.kit_type?.replace('_', ' ') || 'Standard Kit'}
                        </Badge>
                      </div>
                    </div>
                  </DialogHeader>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 mt-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                      ...
                      {/* Components */}
                      {parseComponents(selectedKit.components)?.main_components && (
                        <div>
                      ...
                      {/* Pricing Tiers */}
                      {parseComponents(selectedKit.components)?.pricing_tiers && (
                        <div>
                          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-foreground">
                              ...
                              <Card key={index} className="p-4 bg-muted/30 border-border">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    {tier.description && (
                                      <div className="text-sm text-muted-foreground mt-1">{tier.description}</div>
                                    )}
                                  </div>
                      ...
                      {/* Services */}
                      {parseComponents(selectedKit.components)?.included_services && (
                        <div>
                          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-foreground">
                    ...
                    <div className="space-y-6">
                      ...
                      <Card className="bg-muted/30 border-border">
                        <CardContent className="p-6 text-center">
                          ...
                          <p className="text-muted-foreground mb-4">Complete kit with installation</p>
                      ...
                      <Card className="bg-background border-border">
                        ...
                        <CardContent className="space-y-4">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-muted-foreground">Installation Time:</span>
                            <span className="text-foreground">{selectedKit.installation_time_hours ? `${selectedKit.installation_time_hours}h` : 'Contact us'}</span>
                          </div>
                          {selectedKit.installation_complexity && (
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-medium text-muted-foreground">Complexity:</span>
                              <Badge className={getComplexityColor(selectedKit.installation_complexity)}>
                                {selectedKit.installation_complexity}
                              </Badge>
                            </div>
                          )}
                          {selectedKit.kit_type && (
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-foreground capitalize">{selectedKit.kit_type.replace('_', ' ')}</span>
                            </div>
                          )}
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-muted-foreground">Warranty:</span>
                            <span className="text-foreground">{selectedKit.warranty_months || 12} months</span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      {selectedKit.recommended_crops && selectedKit.recommended_crops.length > 0 && (
                        <Card className="bg-background border-border">
                          ...
                          <CardContent>
                            <div className="flex flex-wrap gap-2">
                              {selectedKit.recommended_crops.map((crop, index) => (
                                <Badge key={index} variant="outline" className="text-muted-foreground border-border">
                                  {crop}
                                </Badge>
                              ))}
                            </div>
                      ...
                      <Card className="bg-background border-border">
                        ...
                        <CardContent className="space-y-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-foreground">Water efficient irrigation</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <CheckCircle className="h-4 w-4 text-primary" />
                            <span className="text-sm text-foreground">Professional installation</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <CheckCircle className="h-4 w-4 text-primary" />
                            <span className="text-sm text-foreground">Training & support included</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <CheckCircle className="h-4 w-4 text-primary" />
                            <span className="text-sm text-foreground">{selectedKit.warranty_months || 12} months warranty</span>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
                  ...
                  <Card className="mt-8 bg-muted/30 border-border">
                    <CardContent className="p-6 text-center">
                      ...
                      <p className="text-muted-foreground mb-6">
                        Our irrigation specialists will help you choose the right system and provide professional installation.
                      </p>
