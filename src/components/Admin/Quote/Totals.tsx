...
export const TotalsSection: React.FC<TotalsSectionProps> = ({
  ...
  const totalVat = items.reduce((sum, item) => {
    const hasItemVat =
      item.include_vat !== undefined &&
      item.include_vat &&
      item.vat_rate !== undefined &&
      item.vat_rate > 0;
    const vatRate = hasItemVat
      ? item.vat_rate / 100
      : quote.include_vat && quote.vat_rate > 0
        ? quote.vat_rate / 100
        : 0;
    return (
      sum +
      (hasItemVat || (quote.include_vat && quote.vat_rate > 0)
  ...
  const displayVatRate = (() => {
    const itemWithVat = items.find(
      (item) =>
        item.include_vat && item.vat_rate !== undefined && item.vat_rate > 0,
    );
    const vatRate = itemWithVat
      ? itemWithVat.vat_rate
      : quote.include_vat && quote.vat_rate > 0
        ? quote.vat_rate
        : 0;
    ...
    <>
      ...
      <div className="block lg:hidden mb-8">
        <div className="w-full bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h4 className="font-semibold text-blue-900 dark:text-blue-400 mb-3 text-center">
            Summary
          </h4>
