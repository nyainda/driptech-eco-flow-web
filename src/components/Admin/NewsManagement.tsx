  ...
  const fetchStats = async () => {
    try {
      ...
      if (!error && Array.isArray(data)) {
        ...
        const totalViews = articlesArray.reduce(
          (sum, article) => sum + (article.views || 0),
          0,
        );
        const totalLikes = articlesArray.reduce(
                ...
                <div className="flex flex-col max-h-[90vh]">
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-3 p-4 sm:p-6 border-t border-border flex-shrink-0">
                    <Button
                      ...
                      className="order-2 sm:order-1 border-border text-muted-foreground hover:bg-muted"
                    >
                      Cancel
                    </Button>
                    <Button
                      onClick={handleCreateOrUpdateArticle}
                      className="order-1 sm:order-2 bg-primary hover:bg-primary/90 text-primary-foreground"
                    >
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              articles.map((article, index) => (
                <Card
                  key={article.id}
                  className="bg-background border-border shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 group"
                >
                  <CardContent className="p-4 sm:p-6">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        <div className="flex-1 space-y-3 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                            ...
                            <Badge
                              variant={
                                article.published ? "default" : "outline"
                              }
